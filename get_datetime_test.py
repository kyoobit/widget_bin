...
def test_resolve_input_ISO_86001_format_w_offset():
    ...
    assert result.minute == 31
    assert result.second == 30
    assert result.tzinfo == datetime.timezone(
...
def test_resolve_input_DayC_Month_ddC_YYYY_HH_MM_SS_format():
    dt = DT()
    result = dt.resolve_input(
        "Friday, February 13, 2009 23:31:30"
...
def test_resolve_input_DayC_dd_Month_YYYY_HH_MM_Z_format():
    dt = DT()
    result = dt.resolve_input(
        "Fri, 13 February 2009 23:31 PST"
    )  # Day, dd Month YYYY HH:MM
    print(f"result {type(result)}: {result!r}")
    assert isinstance(result, datetime.datetime) is True
    assert result.year == 2009
...
def test_resolve_input_Month_dd_HH_MM_YYYY_Z_format():
    dt = DT()
    result = dt.resolve_input(
        "February 13 23:31 2009 ET", debug=True
    )  # Month dd HH:MM YYYY
    print(f"result {type(result)}: {result!r}")
    assert isinstance(result, datetime.datetime) is True
    assert result.year == 2009
...
def test_resolve_input_dd_Month_YYYY_HH_MM_SS_Z0_format():
    dt = DT()
    result = dt.resolve_input(
        "21 July 2023 19:32:01 UTC+12", debug=True
...
def test_resolve_input_dd_Month_YYYY_HH_MM_SS_Z1_format():
    dt = DT()
    result = dt.resolve_input(
        "21 July 2023 19:32 UTC+1200", debug=True
    )  # dd Month YYYY HH:MM[:SS] [Z]
    print(f"result {type(result)}: {result!r}")
    assert isinstance(result, datetime.datetime) is True
    assert result.year == 2023
...
def test_resolve_input_dd_Month_YYYY_HH_MM_SS_Z2_format():
    dt = DT()
    result = dt.resolve_input(
        "21 July 2023 19:32 UTC+12:30", debug=True
    )  # dd Month YYYY HH:MM[:SS] [Z]
    print(f"result {type(result)}: {result!r}")
    assert isinstance(result, datetime.datetime) is True
    assert result.year == 2023
...
def test_resolve_input_no_pattern_matched():
    ...
    with pytest.raises(ValueError):
        dt.resolve_input("this is not a known datetime")
...
def test_resolve_input_zoneinfo_NotFoundError():
    ...
    # Month dd HH:MM[:SS] Z YYYY
    # r"{month}\s{date}\s{hour}:{minute}{second}{meridiem}{tzinfo}\s{year}"
    #
    ...
    # r"{day},\s{month}\s{date}\s{year}\s{hour}:{minute}{second}{meridiem}{tzinfo}"
    #
    ...
    # r"{day},\s{date}\s{month}\s{year}\s{hour}:{minute}{second}{meridiem}{tzinfo}"
    #
    ...
    # r"{month}\s{date}\s{hour}:{minute}{second}{meridiem}\s{year}{tzinfo}"
    #
    ...
    # dd Month YYYY HH:MM[:SS] [Z]
    # r"{date}\s{month}\s{year}\s{hour}:{minute}{second}{tzinfo}"
    #
    # HH:MM[:SS][am|pm][ Z]
    # r"{hour}:{minute}{second}{meridiem}{tzinfo}"
    #
