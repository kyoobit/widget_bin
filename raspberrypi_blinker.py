...
if [[ -z "${BLINKER}" ]]; then
    ...
    tmux new-session -s raspberrypi_blinker -d 'python3 raspberrypi_blinker.py --peak 14 --quiet-hours 04:00-11:00'
fi
...
def get_delay(hour:int, minute:int, offset_map:dict) -> float:
    ...
    logging.debug(f"get_delay selected: uniform({min_delay:03.1f}, {max_delay:06f}) = {delay}")
    return delay
...
def quiet_standby_mode(leds, quiet_hours, delay_between_time_checks=10):
    ...
    logging.debug(f"quiet_standby_mode - in a quiet period: {(now > start and now < end)}")
    ...
    while (now > start and now < end):
        ...
        logging.debug(f"quiet_standby_mode - check time again in {delay_between_time_checks} seconds: {now}")
...
if __name__ == '__main__':
    parser = ArgumentParser(
        ...
        epilog = """Example: 
  tmux new-session -s asyncio_blinker -d 'python3 raspberrypi_blinker.py --peak 14'
        """,
        )

    ## Options for controlling the blinking LEDs
    parser.add_argument('--peak', type=int, metavar='int', default=15,
        help='UTC hour of minimal delay between toggles (default=15)')
    parser.add_argument('--delay', type=float, metavar='float', default=0.1,
        help='minimal delay in seconds between toggles (default=0.1)')
    parser.add_argument('--workers', type=int, metavar='int', default=3,
        help='number of workers to perform toggles (default=3)')
    parser.add_argument('--loops', type=int, metavar='int', default=-1,
        help='number of loops to run (default=-1)')
    parser.add_argument('--quiet-hours', metavar='HH:MM-HH:MM', default='05:00-11:00',
        help='enter a quiet standby mode between UTC hours, \
        adjusted for daylight savings (default=05:00-11:00)')
    parser.add_argument('--leds', metavar='<GPIO##>,<GPIO##>,...',
    ...
    parser.add_argument('--skip-post', action='store_true', help='skip power on self test')
